...
mkTile :: T.Text -> DGT.GoogleTileCoords -> Pixels -> Pixels -> DG.GeoFeatureCollection A.Value -> IO BS8.ByteString
mkTile l zxy buffer quantizePixels geoJson = do
  ...
  where
    ...
    config = DGT.mkConfig l (DGT._gtcZoom zxy) (DGT._coordsX coords, DGT._coordsY coords) (DGT.Pixels $ _pixels buffer) (DGT.Pixels $ _pixels defaultTileSize) (DGT.Pixels $ _pixels quantizePixels)
