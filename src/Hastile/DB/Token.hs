...
getTokensQuery :: HQ.Query () [Token.TokenAuthorisation]
getTokensQuery =
    HQ.statement sql HE.unit (HD.rowsList Token.tokenDecoder) False
  where
    sql = "SELECT token, layers FROM tokens;"

getTokens :: MonadIO m => String -> P.Pool -> m (Either Text.Text [Token.TokenAuthorisation])
...
getTokenQuery :: HQ.Query Text.Text Token.Layers
getTokenQuery =
    HQ.statement sql (HE.value HE.text) (HD.singleRow Token.layersDecoder) False
  where
    sql = "SELECT layers FROM tokens WHERE token LIKE $1;"

getToken :: MonadIO m => String -> P.Pool -> Text.Text -> m (Either Text.Text Token.Layers)
...
updateOrInsertTokenQuery :: HQ.Query Token.TokenAuthorisation ()
...
updateOrInsertToken :: MonadIO m => String -> P.Pool -> Token.TokenAuthorisation -> m (Either Text.Text ())
updateOrInsertToken schemaName pool tokenAuthorisation =
  runDBeither pool action
  where
    action =
      schemaSession schemaName >>
      HS.query tokenAuthorisation updateOrInsertTokenQuery
